...
import NextPage from "@/components/admin/nextPage";
...
export default function XferJobResultList({currentPath, setCurrentPath, title, currentJob, showOff, funcs, back}) {    
  ...
  async function getList(cache, refresh) {
    if (!mount || !currentJob) return
    getResourcesWith(cache, refresh, 'jobs/' + currentJob?.id + '/result', 50, mount, token, false, setList, count, setCount, null, null, curmount?.curdomainid, moreMeta)
  }

  async function refreshList() {
    if (!mount || !currentJob) return
    getResourcesWith(false, true, 'jobs/' + currentJob?.id + '/result', 50, mount, token, false, setList, count, setCount, null, null, curmount?.curdomainid, moreMeta)
  }
  ...
  async function goSub(record) {
      if (!curmount) return
        ...
        currentPath.storage = currentJob?.spec.storage
        currentPath.meta = pathlib.normalize("/"+record.path)
        
        curmount.curstorageid = currentJob?.spec.storage
        setCount({
          meta: count.meta
        })
...
<FilesNav title={title} page={title+"xferJobResultListRef"} back={back}/>
  ...
  <div className="relative py-2 z-0 w-full mb-4 group border-b border-neutral-300">
    ...
    <div>
      ...
      <div className="flex flex-row gap-4">
        ...
        <div>
          Storage: {currentJob?.spec.storage}
        </div>
        <div>
