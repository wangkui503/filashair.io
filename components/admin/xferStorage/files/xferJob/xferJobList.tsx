...
export default function XferJobList({title, setCurrentJob, back}) {    
  ...
  async function refreshList() {
    ...
    getResourcesWith(false, true, 'jobs', 5, mount, token, false, setList, count, setCount, null, curmount?.curstorageid, curmount?.curdomainid, moreMeta)
  }
  ...
  async function getList(cache, ascend) {
    ...
    getResourcesWith(cache, false, 'jobs', 5, mount, token, ascend, setList, count, setCount, null, curmount?.curstorageid, curmount?.curdomainid, moreMeta)
  }
  ...
  function setupPolling(interval) {
    ...
    const pollingThread = setInterval(() => {  
        ...
        console.log("storage getLatestJobs polling--------------",mount?.jobs?.filashnexttoken)        
    }, interval)
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      ...
      if (entry.isIntersecting) {
        console.log("XferJobList isIntersecting-- ", entry)
        getList(true, false)
        //observer.disconnect();
    ...
    }, [curmount?.token, curmount?.curstorageid])
  ...
  function filter() {
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      filtered = filtered.filter(item=> item.status === pattern || item.spec.action === pattern || item.spec.storage?.includes(pattern) || item.spec.path?.includes(pattern) || item.spec.name?.includes(pattern))
    }
...
<div ref={listRef} className="pb-4 bg-white ">
  <FilesNav title={title} page={title+"xferJobListRef"} back={back}/>
