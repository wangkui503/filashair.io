...
export default function FilesCard({i, parentPath, parentType, file, goSub, showOff, funcs, deleteFunc}) {    
  ...
  async function switchToNewShare(file) {
    if (!curmount) return
    const key = "storage_" + curmount?.curstorageid
    curmount.curstoragepath = pathlib.join(parentPath, file.name)
    /* setCount({
      meta: count.meta
    }) */
    tabSwitch('shareNewRef', 'auto')
  }


    return (
        <div className={`${i%2 === 0 ? 'bg-stone-100':''} ${file.type === 'folder' ? 'font-semibold':'font-normal'} w-full grid md:grid-cols-12 justify-start border-b md:hover:shadow-lg md:hover:bg-stone-200`}>
  
        <div className="flex flex-row md:col-span-5 justify-start p-3 leading-normal whitespace-nowrap overflow-x-auto scrollbar-hide">
                <button 
          ...
          <div className="flex flex-col md:col-span-2 justify-start p-3 leading-normal whitespace-nowrap overflow-x-auto scrollbar-hide">
            <p className="text-gray-700">
            {file.size}
            </p>
          </div>
          <div className="flex flex-col md:col-span-3 justify-start p-3 leading-normal whitespace-nowrap overflow-x-auto scrollbar-hide">
            <p className="text-gray-700">
            {file.mtime}
            </p>
          </div>
          ...
          <div ref={funcsRef} className={`${showOff ? '' : 'hidden'} whitespace-nowrap overflow-x-auto scrollbar-hide w-full flex flex-row gap-2 md:col-span-${funcs?.length+2} justify-start p-3 leading-normal`}>
          {
            funcs?.map((func, i) => {
              return (
                <div key={i} >
                <button
                  onClick={()=>{func.action(session, curmount, (parentType === 'folder' ? parentPath : pathlib.dirname(parentPath)+'/'), file);funcsRef.current?.classList.add('hidden')}}
