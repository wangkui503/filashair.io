...
import XferPolicyPlug from "./XferPolicyPlug";
import XferPolicyNew from "./XferPolicyNew";
...
import XferPolicyNav from "./xferPolicyNav";
import NextPage from "../nextPage";


export default function XferPolicy() {
  ...
  function setupPolling(interval) {
    const pollingThread = setInterval(() => {  
      if (!mount?.[resourceKind]) return
        ...
        console.log("policy polling--------------",mount?.[resourceKind]?.filashnexttoken)        
    }, interval)
  ...
  function filter() {
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      filtered = filtered.filter(item=> item.id?.includes(pattern) || item.subject?.includes(pattern) || item.share?.includes(pattern) || item.permissions.includes(pattern) || item.last_updated_by?.includes(pattern) || item.last_updated_at?.includes(pattern))
    }
...
<div id="policiesList" ref={listRef} className="pb-4 bg-white ">
  <XferPolicyNav page="sharedToMeRef"/>    
  </div>
    ...
    return (
      ...
      <div className="flex flex-row justify-start p-3 leading-normal overflow-x-auto">
        ...
        <div className="mb-6 flex flex-row gap-3">
          <div>
                <input
                checked={policy.permissions.includes('read')}
                onChange={(e)=>updatePolicyPer(e, 'read', policy)}
            type="checkbox"
          ...
          <div>
              <input
              checked={policy.permissions.includes('write')}
              onChange={(e)=>updatePolicyPer(e, 'write', policy)}
          type="checkbox"
          className="mr-3 w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded  "
        /> Write
        </div>
        <div>
              <input
              checked={policy.permissions.includes('list')}
              onChange={(e)=>updatePolicyPer(e, 'list', policy)}
          type="checkbox"
          className="mr-3 w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded  "
        /> List
        </div>
        <div>
              <input
              checked={policy.permissions.includes('read') && policy.permissions.includes('write') && policy.permissions.includes('list')}
              onChange={(e)=>updatePolicyAllPers(e, policy)}
          type="checkbox"
          className="mr-3 w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded  "
        /> All
        </div>
