...
import FilesJobListCard from "./filesJobListCard";


export default function XferJobList({title, setCurrentJob, back}) {    
  ...
  const [pollingPaging, setPollingPaging] = useState({meta:0});
  const newPolling = () => setPollingPaging({meta: pollingPaging.meta+1})
  ...
  async function getList(cache, ascend) {
    if (!curmount?.curshareid) {
      setList([])
      return
    }
    getResourcesWith(cache, false, 'jobs', 5, mount, token, ascend, setList, count, setCount, curmount?.curshareid, null, null, moreMeta)
  }
  ...
  async function refreshList() {
    ...
    getResourcesWith(false, true, 'jobs', 5, mount, token, false, setList, count, setCount, curmount?.curshareid, null, null, moreMeta)
  }
  ...
  function setupPolling(interval) {
    ...
    const pollingThread = setInterval(() => {  
        newPolling()
        console.log("getLatestJobs polling--------------",mount?.jobs?.filashnexttoken)        
    }, interval)
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      ...
      if (entry.isIntersecting) {
        console.log("XferJobList IntersectionObserver isIntersecting-- ", entry)
        getList(true, false)
        //observer.disconnect();
        pollingThread = setupPolling(2000);
      } else {
        console.log("XferJobList IntersectionObserver leaving-- ", entry)
    ...
    }, [curmount?.token, curmount?.curshareid])
  ...
  function filter() {
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      filtered = filtered.filter(item=> item.status === pattern || item.spec.action === pattern || item.spec.share?.includes(pattern) || item.spec.path?.includes(pattern) || item.spec.name?.includes(pattern))
    }
...
<div ref={listRef} className="pb-4 bg-white ">
  <FilesNav title={title} page={"xferJobListRef"} back={back}/>
...
<div className="w-full grid md:grid-cols-11 justify-start border-b">
...
{
  (count.meta?.curmountid && !curmount?.token) ? 'loading...' :
  filter()?.map((job, i) => {
    return (
      <FilesJobListCard key={i} title={title} job={job} i={i} setCurrentJob={setCurrentJob}/>
    )
  })
}
