...
export default function XferJobResultList({currentPath, setCurrentPath, title, currentJob, showOff, funcs, back}) {    
  ...
  async function getList(refresh) {
    if (!mount || !currentJob) return
    getResourcesWith(false, refresh, 'jobs/' + currentJob?.id + '/result', 50, mount, token, false, setList, count, setCount, curmount?.curshareid, null, curmount?.curdomainid, moreMeta)
  }

  async function refreshList() {
    if (!mount || !currentJob) return
    getResourcesWith(false, true, 'jobs/' + currentJob?.id + '/result', 50, mount, token, false, setList, count, setCount, curmount?.curshareid, null, curmount?.curdomainid, moreMeta)
  }



  useEffect(() => {
    console.log("XferJobResultList is launching-----", currentJob)
    if(!currentJob) return
    getList(true)    
  }, [currentJob])

  async function goSub(record) {
      if (!curmount) return
      else {
        const exist = curmount?.shares?.filter((share)=>share.id == currentJob?.spec.share)[0]
        if (!exist) {
          toast.error('the share is not loaded')
          return
        }

        currentPath.share = currentJob?.spec.share
        currentPath.meta = pathlib.normalize("/"+record.path)
        
        curmount.curshareid = currentJob?.spec.share
        setCount({
          meta: count.meta
        })
        setCurrentPath({meta:currentPath.meta})
        console.log('job result gosub--', count)
        tabSwitch(title+'filesListRef', 'auto')      
      }
      console.log("job gosub---", currentPath.meta)
  ...
  function filter() {
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      if (pattern.toLowerCase().startsWith("$size")) {
        ...
        filtered = filtered?.filter(file => file.type === pattern || file.size == pattern || file.name?.includes(pattern) || file.path?.includes(pattern) || file.mtime?.includes(pattern))
      }      
    }
...
<FilesNav title={title} page={"xferJobResultListRef"} back={back}/>
  ...
  <div className="relative py-2 z-0 w-full mb-4 group border-b border-neutral-300">
    ...
    <div>
      ...
      <div className="flex flex-row gap-4">
        <div>
          Action: {currentJob?.spec.action}
        </div>
        <div>
          Share: {currentJob?.spec.share}
        </div>
        <div>
          Path: {currentJob?.spec.path}
        </div>
        <div>
          Keyword: {currentJob?.spec.name}
        </div>
      </div>
    </div>
    <div>
      ...
      <div className="flex flex-row gap-4">
        <div>
          total size: {currentJob?.progress.total_size}
        </div>
        <div>
          found files: {currentJob?.progress.found}
        </div>
        <div>
          regular files: {currentJob?.progress.regulars}
        </div>
        <div>
          folders: {currentJob?.progress.folders}
        </div>
      </div>
    </div>    
  </div>
...

{
  filter()?.map((record, i) => {
    return (
      <div key={i} className={`${i%2 === 0 ? 'bg-stone-100':''} ${record.type === 'folder' ? 'font-semibold':'font-normal'} w-full grid md:grid-cols-8 justify-start border-b md:hover:shadow-xl md:hover:bg-slate-200`}>

      <div className="overflow-auto scrollbar-hide flex flex-row md:col-span-2 justify-start p-3 leading-normal overflow-x-auto">
          <button className="whitespace-nowrap font-semibold tracking-tight text-gray-900 "
            
            onClick={()=>goSub(record)}>
              {record.name}            
          </button>        
        </div>
        <div className="overflow-auto scrollbar-hide flex flex-row md:col-span-2 justify-start p-3 leading-normal overflow-x-auto">
          <button className="whitespace-nowrap text-gray-700"
