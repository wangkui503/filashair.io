...
import XferXferNav from "./xferXferNav";
import XferListCard from "./xferListCard";

const final_statuses = ['completed', 'aborted']

export default function XferXfer({setDetailTransfer}) {
  ...
  useEffect(() => {
    ...
    getLatestTransfers(false, false)
  }, [paging]); 
  

  
  async function getLatestTransfers(cache, ascend) {
    if (!mount || !token) return
    getResourcesWith(cache, false, 'transfers', 5, mount, token, ascend, setList, count, setCount, null, null, null, moreMeta)
    currentMount.meta = curmount  
  }

  async function refreshList() {
    if (!mount || !token) return
    getResourcesWith(false, true, 'transfers', 5, mount, token, false, setList, count, setCount, null, null, null, moreMeta)
    currentMount.meta = curmount  
  }

  function setupPolling(interval) {
    const pollingThread = setInterval(() => {      
      if (!mount?.transfers) return
      getLatestTransfers(false, true)
      
        console.log("getLatestTransfers polling--------------",mount?.transfers?.filashnexttoken)        
    }, interval)
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      ...
      if (entry.isIntersecting) {
        console.log("xferXfer isIntersecting-- ", entry)
        getLatestTransfers(true, false)
        observer.disconnect();
      }
    },
    {
    ...
    const pollingThread = setupPolling(2000);
    const pollingStatusThread = setupPollingTransferStatus(2000);    
    return () => {
      observer.disconnect();
  ...
  async function doPollTransfer(xfer) {
    if (!mount || final_statuses.includes(xfer.status)) return
    fetch(mount.host + '/transfers/' + xfer.id, {
      method: "GET",
      headers: {
      ...
      if (res.status === 200) {
        ...
        console.log("doPollTransfer result--------------", result)        
  ...
  async function doPollTransfers() {
    if (!mount) return
    mount?.transfers?.all?.map(xfer => {
  ...
  function setupPollingTransferStatus(interval) {
    doPollTransfers()
    const pollingThread = setInterval(() => {      
        doPollTransfers()
        console.log("setupPollingTransferStatus polling--------------")        
    }, interval)
    return pollingThread       
  }
  ...
  function filter() {
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      filtered = filtered.filter(item=> item.spec?.action === pattern || item.status === pattern || item.progress?.progress === pattern || item.spec.source.share?.includes(pattern) || item.spec.remote_address?.includes(pattern) || item.spec.dest.share?.includes(pattern) || item.last_updated_at?.includes(pattern))
    }
...
<div ref={listRef} className="pb-4 bg-white ">
  <XferXferNav page="xferTransferListRef"/>    
  </div>
...
<div className="w-full grid md:grid-cols-9 justify-start gap-4 border-b">
...
{  currentMount.meta != curmount ? 'loading...' :   
  filter()?.map((xfer,i) => {
    return (
      <XferListCard key={i} transfer={xfer} i={i} setDetailTransfer={setDetailTransfer}/>
    )
  })
}
...
<NextPage more={moreMeta} newLimit={newLimit} monitors={[count.meta?.curmountid]}/>
