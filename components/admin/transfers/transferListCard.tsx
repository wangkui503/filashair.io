...
export default function TransferListCard({transfer, i, provisionInviteMessage, setDetailTransfer}) {    
  ...
  async function getXferLoc(transfer, xfer, loc) {    
    ...
    if (res.status === 200) {
      ...
      xfer.error = res.status
      const updated = new Date(transferCard.meta.updatedat)
      const now = new Date();
  ...
  async function updateTransferStatus(transfer) {
    ...
    fetch("/api/transfers/"+session?.user?.email, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },    
  ...
  const dests = specsObj.map(xfer => xfer.spec?.dest?.path)

  
    return (
        
  <div className={`${transferCard.meta.error ? 'bg-red-100' : i%2 === 0 ? 'bg-stone-100':''} ${transferCard.meta.direction === 'upload' ? 'font-semibold':'font-normal'} w-full grid md:grid-cols-8 gap-3 justify-start border-b md:hover:shadow-xl md:hover:bg-slate-200`}>
        ...
        <div className="flex flex-col justify-start py-3 leading-normal">
          <button 
              onClick={()=>{setDetailTransfer(transferCard); tabSwitch('detailTransferRef', 'auto')}}
          className="whitespace-nowrap overflow-x-auto scrollbar-hide whitespace-nowrap text-gray-500">
          {transferCard.meta.message}
          </button>          
        </div>
        ...
        <div className="flex flex-row gap-6 justify-start py-3 leading-normal">
          <button onClick={()=> {setDetailTransfer(transferCard.meta);tabSwitch('detailTransferRef','auto')}}>
