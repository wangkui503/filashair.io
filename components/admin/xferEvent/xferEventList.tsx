...
import XferEventNav from "./xferEventNav";
import NextPage from "../nextPage";
import { getResourcesWith } from "@/lib/getXferAccount";

export default function XferEventList() {    
  ...
  async function getList(cache, ascend) {
    ...
    getResourcesWith(cache, false, resourceKind, 5, mount, token, ascend, setList, count, setCount, curmount?.curshareid, null, curmount?.curdomainid, moreMeta)
    currentMount.meta = curmount  
  }

  async function refreshList() {
    if (!mount || !token) return
    getResourcesWith(false, true, resourceKind, 5, mount, token, false, setList, count, setCount, curmount?.curshareid, null, curmount?.curdomainid, moreMeta)
    currentMount.meta = curmount  
  }
  ...
  function setupPolling(interval) {
    const pollingThread = setInterval(() => {  
        ...
        console.log("event polling--------------",mount?.[resourceKind]?.filashnexttoken)        
    }, interval)
  ...
  function filter() {
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      filtered = filtered.filter(item=> item.resource === pattern || item.method === pattern || JSON.stringify(item.subject)?.includes(pattern) || item.created_at?.includes(pattern))
    }
...
<div ref={listRef} className="pb-4 bg-white ">
  <XferEventNav page="sharedToMeRef"/>    
  </div>
