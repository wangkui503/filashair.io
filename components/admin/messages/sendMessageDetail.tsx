...
export default function SendMessageDetail({detailSendMessage, back}) {    
  ...
  if (!detailSendMessage) return
  const xfers = detailSendMessage?.xfers?? JSON.parse(detailSendMessage?.specs)
  

  console.log("MessageDetail---", detailSendMessage)
  ...
  return (
    <div className="px-3 w-full bg-white  ">
      ...
      <div>
        Message Detail - {detailSendMessage?.subject} - {detailSendMessage?.status}
      </div>
      </div>
        ...
        <div className="flex flex-row relative">
            <div className="overflow-x-auto relative flex flex-row z-0 w-full border-b-2 my-4 group">
              ...
              <span className="pr-2">{detailSendMessage?.tos}</span>
              </div>
              </div>
        ...
        <div className="flex flex-row w-full">
          <div className="relative z-0 w-full mb-4 group">
            <input
              ...
              placeholder=" "
              required
              value={detailSendMessage?.subject}
              disabled
            />
            <label
              htmlFor="subject"
        ...
        <div className="flex flex-row w-full">
          <div className="relative z-0 w-full mb-4 group">
            <textarea
              ...
              className="block py-2 pr-10 w-full border-b-2 bg-transparent appearance-none peer"
              placeholder=" "
              value={detailSendMessage?.message}
              disabled
            />
            <label
              htmlFor="msgtxt"
        ...
        <SpeedChart xfers={xfers} status={detailSendMessage?.status} />
        </div>
